...
class Player extends Database
{
    ...
    public function getRows($start = 0, $limit = 3)
    {
        $sql = "SELECT * FROM {$this->tableName} WHERE estado_Campo= 0 ORDER BY id DESC LIMIT {$start},{$limit}";
        $stmt = $this->conn->prepare($sql);
        $stmt->execute();
        if ($stmt->rowCount() > 0) {
    ...
    {
        $sql = "UPDATE {$this->tableName} SET estado_Campo=1 WHERE id=:id";
        $stmt = $this->conn->prepare($sql);
        try {
            $stmt->execute([':id' => $id]);
            if ($stmt->rowCount() > 0) {
    ...
    {
        $sql = "SELECT * FROM {$this->tableName} WHERE nombre_Campo LIKE :search ORDER BY id DESC LIMIT {$start},{$limit}";
        $stmt = $this->conn->prepare($sql);
