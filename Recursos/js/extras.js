...
function pagination(totalpages, currentpage) {
  ...
  function getplayerrow(player) {
    ...
    if (player) {
      playerRow = `<tr>
              <td class="align-middle">${player.raqueta}</td>
              <td class="align-middle">${player.pelotas}</td>
              <td class="align-middle">${player.uniformes}</td>
              <td class="align-middle">${player.dia}</td>
  ...
  function getplayers() {
    ...
    $.ajax({
      url: "/reservacampo/Controladores/Extra/ajax.php",
      type: "GET",
      dataType: "json",
  ...
  $(document).ready(function () {
    ...
    $(document).on("submit", "#addform", function (event) {
      ...
      var alertmsg =
        $("#userid").val().length > 0
        ? "El equipo adicional se ha actualizado con Éxito!"
        : "Se ha Añadido el nuevo equipo con Éxito!";
      $.ajax({
        url: "/reservacampo/Controladores/Extra/ajax.php",
        type: "POST",
        dataType: "json",
    ...
    $(document).on("click", "a.edituser", function () {
      ...
      $.ajax({
        ...
        success: function (player) {
          if (player) {
            $("#raquetas").val(player.raqueta);
            $("#pelotas").val(player.pelotas);
            $("#uniformes").val(player.uniformes);
            $("#dia").val(player.dia);
            $("#cbx_campo").val(player.reservas_idReservas);
    ...
    $(document).on("click", "a.deleteuser", function (e) {
      e.preventDefault();
      var pid = $(this).data("id");
      if (confirm("¿Está seguro que quiere elimar el equipo Adicional?")) {
        $.ajax({
          url: "/reservacampo/Controladores/Extra/ajax.php",
          type: "GET",
          dataType: "json",
          ...
          success: function (res) {
            if (res.deleted == 1) {
              $(".message")
                .html("El equipo Adicional se ha eliminado con Éxito!")
                .fadeIn()
                .delay(3000)
                .fadeOut();
              getplayers();
          ...
          if (player) {
            ...
            const profile = `<div class="row">
                    ...
                    <div class="col-sm-6 col-md-8">
                      <h4 class="text-primary">${player.raqueta}</h4>
                      <p class="text-secondary">
                        <i class="fa fa-envelope-o" aria-hidden="true"></i> ${player.pelotas}
                        <br />
                        <i class="fa fa-phone" aria-hidden="true"></i> ${player.resevas_idReservas}
                      </p>
                    </div>
                  </div>`;
